...
namespace VanHanhCD1.Controllers.ThieuKet
{
    ...
    [ApiController]
    public class DongCoThieuKet2Controller : ControllerBase
    {
        private readonly IThieuKetRepository _service;
        public DongCoThieuKet2Controller(IThieuKetRepository service)
        {
            _service = service;
        }
        [HttpGet("last-24h")]
        public async Task<IActionResult> GetLast24HoursDongCoThieuKet2s()
        {
            var result = await _service.GetLast24HoursDongCoThieuKet2s();
            return Ok(result);
        }
        [HttpGet("min-value")]
        public IEnumerable<object> GetMinValue()
        {
            var result = _service.GetDongCoThieuKet2MinValues();
            return result;
        }
        [HttpGet("search")]
        public async Task<IActionResult> SearchByTimeRangeDongCoThieuKet2s([FromQuery] DateTime from, [FromQuery] DateTime to)
        {
            var result = await _service.GetSearchTimeDongCoThieuKet2s(from, to);
            return Ok(result);
        }
        //[HttpGet("export")]
